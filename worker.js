...
/*
 * Промокоды с наценкой (увеличивают цену) — например, для теста.
 */
const SURCHARGE_PROMOS = {
  "3abuctb": 1.00 // +100% к цене
};

/*
 * Секретный промокод для проверки работоспособности оплаты.
 * Требует отдельный пароль — без него скидка не применяется,
 * даже если кто-то угадает сам код.
 */
const SECRET_PROMO = {
  code: "foxikchmo211",
  password: "21128900990",
  discount: 0.99
};
  ...
  async fetch(request, env) {
    ...
    if (
      ...
      try {
        ...
        if (promoCode === SECRET_PROMO.code) {
          const providedPassword = String(body.secretPassword || "").trim();

          if (providedPassword !== SECRET_PROMO.password) {
            return json(
              {
                success: false,
                error: "Неверный пароль для промокода"
              },
              403,
              origin
            );
          ...
          amount =
            Math.round(
              amount * (1 - SECRET_PROMO.discount) * 100
            ) / 100;
        } else if (SURCHARGE_PROMOS[promoCode] !== undefined) {
          amount =
            Math.round(
              amount * (1 + SURCHARGE_PROMOS[promoCode]) * 100
            ) / 100;
        } else if (PROMOS[promoCode]) {
          amount =
            Math.round(
